...
class ClientThread implements Runnable {
	...
	public void run() {
		...
		try {
			...
			write(out, "system:admin", Charset.UTF8);
			System.out.println("get login data");
			String message = read(in, Charset.UTF8);
			...
			while (true) {
				...
				if (i >= 90) {
					...
					write(out, "heart", Charset.UTF8);
					continue;
				}
				write(out, data, Charset.UTF8);
				System.out.println("begin read message from server.");
				message = read(in, Charset.UTF8);
				System.out.println(message);
			}
